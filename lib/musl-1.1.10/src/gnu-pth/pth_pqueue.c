...
intern void pth_pqueue_insert(pth_pqueue_t *q, int prio, pth_t t)
{
    ...
    pth_assert(q != &LSCHED->pth_RQ || t->state != PTH_STATE_SCHEDULER);
...
intern void pth_pqueue_delete(struct pth_pqueue_st *q, pth_t t)
{
    if (q == NULL)
	return;
    	//q = t->pqueue;
    ...
    if (t == NULL)
        return;
        //pth_debug1("Removing a thread that does not belong to the list!");
    else if (q->q_head == t) {
        if (t->q_next == t) {
            /* remove the last element and make queue empty */
            t->q_next = NULL;
