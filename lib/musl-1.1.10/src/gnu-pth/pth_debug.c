...
intern void pth_debug(const char *file, int line, int argc, const char *fmt, ...)
{
    ...
    pth_shield {
        va_start(ap, fmt);
        if (file != NULL)
            pth_snprintf(str, sizeof(str), "%d:%d:%s:%04d: ", LSCHED_ID, (int)getpid(), file, line);
        else
            str[0] = NUL;
        n = strlen(str);
        if (argc == 1)
...
intern void pth_dumpstate(FILE *fp)
{
    ...
    pth_dumpqueue(fp, "NEW", &LSCHED->pth_NQ);
    pth_dumpqueue(fp, "READY", &LSCHED->pth_RQ);
    fprintf(fp, "| Thread Queue RUNNING:\n");
    fprintf(fp, "|   1. thread 0x%lx (\"%s\")\n",
            (unsigned long)pth_current, pth_current->name);
    pth_dumpqueue(fp, "WAITING", &LSCHED->pth_WQ);
    pth_dumpqueue(fp, "SUSPENDED", &LSCHED->pth_SQ);
    pth_dumpqueue(fp, "DEAD", &LSCHED->pth_DQ);
    fprintf(fp, "+----------------------------------------------------------------------\n");
    return;
}
