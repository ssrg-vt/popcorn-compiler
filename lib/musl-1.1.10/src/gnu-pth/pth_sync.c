...
int pth_mutex_init(pth_mutex_t *mutex)
{
    ...
    mutex->mx_owner = NULL;
    mutex->mx_count = 0;
    pthread_spin_init(&mutex->mx_splock, 1);
    return TRUE;
}
...
retry:
    if(pthread_spin_lock(&mutex->mx_splock))			//<-----
        return pth_error(FALSE, EBUSY);//TODO: fix error code
	...
	pthread_spin_unlock(&mutex->mx_splock);			//----->
    ...
    for (;;) {
        ...
        if (!(mutex->mx_state & PTH_MUTEX_LOCKED))
	{
            pth_debug1("pth_mutex_acquire: retrying mutex");
            goto retry;
	}
    }
...
int pth_cond_await(pth_cond_t *cond, pth_mutex_t *mutex, pth_event_t ev_extra)
{
    ...
    /* safe since mutex is already held */
