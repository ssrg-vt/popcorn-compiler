...
#define PRINTERR(...) do { \
    fprintf(stdout, "[dumb-init] " __VA_ARGS__); \
} while (0)
...
void forward_signal(int signum) {
    ...
    if (signum != 0) {
        ...
        kill(-1, signum);
        DEBUG("Forwarded signal %d to all children.\n", signum);
    } else {
...
int sigpending_next(const sigset_t *set, int *sig)
{
	int ret;
	...
	sigset_t and_set;
    	sigemptyset(&ret_set);
	ret=sigpending(&ret_set);
	if(ret)
		return ret;

	ret = sigandset(&and_set, &ret_set, set);
	if(ret)
		return ret;
	...
	*sig=-1;
	int i;
	for(i=0; i< MAXSIG; i++)
		if(sigismember(&and_set, i))
			*sig=i;
		
	return 0;
}

int sigwait_timed(const sigset_t *set, int *sig)
{
	siginfo_t info;
	struct timespec tmsp;
	tmsp.tv_sec=1;
	tmsp.tv_nsec=0;
	int ret = sigtimedwait(set, &info, &tmsp);
	//printf("%s: ret %d\n", __func__, ret);
	if(ret<0)
		*sig = -1;
	else
...
int main(int argc, char *argv[]) {
    ...
    if (child_pid < 0) {
        ...
        for (;;) {
            ...
            sigwait_timed(&all_signals, &signum);
            //sigpending_next(&all_signals, &signum);
	    //printf("%s: signum %d\n", __func__, signum);
	    if(signum!=-1)
