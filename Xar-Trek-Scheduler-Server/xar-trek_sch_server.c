 ...
 * Copyright (C) 2020-2021 Ho-Ren (Jack) Chuang <horenc@vt.edu>; Edson Horta <edsonh@vt.edu>
 *
...
#define POPCORN_X86 "10.1.1.45" /* change it according to your setup */
...
//	printf("SCHEDULING POLICY BEGINS HERE\n");
	if (!strcmp(command, "END")) { /* process end */
		return;
	}
   strcpy(cmd,"grep -E '\\s*" );
   strcat(cmd,prog_name );
   strcat(cmd, "\\s*,.+,' ~/Xar-Trek/Scheduler/KNL_HW_Sched.txt | cut -f2 -d,");
   ...
   strcpy(cmd,"grep -E '\\s*" );
   strcat(cmd,prog_name );
   strcat(cmd, "\\s*,.+,' ~/Xar-Trek/Scheduler/KNL_HW_Sched.txt | cut -f3 -d,");
   fp = popen(cmd,"r");
   fscanf(fp,"%s", cmd_str);
   ...
   strcpy(cmd,"grep -E '\\s*" );
   strcat(cmd,prog_name );
   strcat(cmd, "\\s*,.+,' ~/Xar-Trek/Scheduler/KNL_HW_Sched.txt | cut -f4 -d,");
   fp = popen(cmd,"r");
   fscanf(fp,"%s", cmd_str);
