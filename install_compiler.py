...
def install_libraries(base_path, install_path, num_threads, st_debug,
    ...
    with open(os.devnull, 'wb') as FNULL:
        ...
        #=====================================================
        # CONFIGURE & INSTALL LIBDWARF
        #=====================================================
        os.chdir(os.path.join(base_path, 'lib/libdwarf'))
        ...
        print("Configuring libdwarf (aarch64)...")
        try:
            cflags = 'CFLAGS="-O3 -ffunction-sections -fdata-sections ' +\
                     '-specs {}"'.format(os.path.join(aarch64_install_path, 
                                                     'lib/musl-gcc.specs'))
           ...
           print('Could not configure libdwarf ({})!'.format(e))
           sys.exit(1)
        else:
           if rv != 0:
               print('libdwarf configure failed.')
               sys.exit(1)

        print('Making libdwarf...')
        try:
            print('Running Make...')
            rv = subprocess.check_call(['make', '-j', str(num_threads)])
            rv = subprocess.check_call(['cp', 'libdwarf.a',
                                        os.path.join(aarch64_install_path,
                                                     'lib')])
            rv = subprocess.check_call(['cp', 'dwarf.h', 'libdwarf.h',
                                        os.path.join(aarch64_install_path,
                                                     'include')])
        except Exception as e:
            print('Could not run Make ({})!'.format(e))
        ...
        print("Configuring libdwarf (x86_64)...")
        try:
            cflags = 'CFLAGS="-O3 -ffunction-sections -fdata-sections ' +\
                     '-specs {}"'.format(os.path.join(x86_64_install_path, 
                                                     'lib/musl-gcc.specs'))
        ...
        try:
            ...
            rv = subprocess.check_call(['make', '-j', str(num_threads)])
            rv = subprocess.check_call(['cp', 'libdwarf.a',
                                        os.path.join(x86_64_install_path,
                                                     'lib')])
            rv = subprocess.check_call(['cp', 'dwarf.h', 'libdwarf.h',
                                        os.path.join(x86_64_install_path,
                                                     'include')])
