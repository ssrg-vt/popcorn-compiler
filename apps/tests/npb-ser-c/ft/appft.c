...
int allocate_arrays() {
	...
	twiddle = malloc(NZ * sizeof(double**));
	for(i=0; i<NZ; i++) {
		twiddle[i] = malloc(NY * sizeof(double *));
		for(j=0; j<NY; j++)
			twiddle[i][j] = malloc((NX+1) * sizeof(double));
	}

	xnt = malloc(NZ * sizeof(dcomplex**));
	for(i=0; i<NZ; i++) {
		xnt[i] = malloc(NY * sizeof(dcomplex *));
		for(j=0; j<NY; j++)
			xnt[i][j] = malloc((NX+1) * sizeof(dcomplex));
	}

	y = malloc(NZ * sizeof(dcomplex**));
	for(i=0; i<NZ; i++) {
		y[i] = malloc(NY * sizeof(dcomplex *));
		for(j=0; j<NY; j++)
			y[i][j] = malloc((NX+1) * sizeof(dcomplex));
	}
