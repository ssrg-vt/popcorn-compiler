...
#define SET_REG64( var, reg ) asm volatile("ld "reg", %0" : : "g" (var) : reg )
#define SET_REG64_x( var1, var2, reg ) asm volatile("l"size"zx "reg", %0,%1" : : "g" (var1), "g" (var2) : reg )
...
#define GET_LR( var )  asm volatile( "mflr %0" : "=r" (var) : : )
#define GET_CTR( var )  asm volatile( "mfctr %0" : "=r" (var) : : )
...
#define SET_PC_REG( val ) asm volatile("mtlr %0;  blr" :: "r" (val) )
...
// Note: do not set PC, SP(r0) and FBP(r31) as they require special handling
...
{ \
  ...
  SET_R31((regset_powerpc64).r[31]); \
  SET_FP_REGS_powerpc64(regset_powerpc64); \
}

/* Get frame information. */
#define GET_FRAME_powerpc64( bp, sp ) GET_R31(bp); GET_R1(sp);
...
#define SET_FRAME_powerpc64( bp, sp ) \
  asm volatile("mr 0, %0; mr 31, %1;" : : "r" (sp), "r" (bp) )
