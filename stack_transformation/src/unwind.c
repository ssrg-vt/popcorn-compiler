...
static inline void restore_callee_saved_regs(rewrite_context ctx, int act)
{
  ...
  ST_INFO("prev fbp: %p [restore_callee_saved_regs]\n",  REGOPS(ctx)->fbp(ctx->acts[act - 1].regs));
  ST_INFO("prev sp: %p [restore_callee_saved_regs]\n",  REGOPS(ctx)->sp(ctx->acts[act - 1].regs));
  ST_INFO("prev ra_reg: %p [restore_callee_saved_regs]\n", REGOPS(ctx)->ra_reg(ctx->acts[act - 1].regs));
  for(i = unwind_start; i < unwind_end; i++)
  {
    ...
    ST_INFO("REGOPS(ctx)->reg(ctx->acts[act].regs --> locs[i].reg): %p [restore_callee_saved_regs]\n", \
...
//  #if defined (__powerpc64__)
//    /* Fix fbp to point to sp for ppc */
//    REGOPS(ctx)->set_fbp(ACT(ctx).regs,(REGOPS(ctx)->sp(ACT(ctx).regs)));
//  #endif
...
void pop_frame_funcentry(rewrite_context ctx)
{
  ...
  ST_INFO("curr frame size:%d [pop_frame_funcentry]\n", ACT(ctx).site.frame_size);
  ST_INFO("next frame size:%d [pop_frame_funcentry]\n", NEXT_ACT(ctx).site.frame_size);
