...
\begin{comment}
TODO -- when we open up the repository to the public, this needs to be changed to clone the repo using anonymous read-only access.
\end{comment}

The Popcorn compiler repository is hosted on SSRG's organization GitHub page, located at \url{https://github.com/ssrg-vt/}.  \textbf{For now, access to the repository is private.  In order to get access, you'll either need to be a part of the compiler team in the SSRG GitHub, or have an SSRG administrator add you as an outside collaborator}.  Contact \href{mailto:rlyerly@vt.edu}{Rob Lyerly} with your name and GitHub username to be added as an outside collaborator.
...
\begin{itemize}
	\item The toolchain requires \prog{gcc} for each ISA to be targeted by multi-ISA binaries.  Currently, compiling \texttt{libelf} requires \prog{gcc}, although in the future it will be compiled with \prog{clang}.  For now, each new supported ISA requires installing \prog{gcc-<ISA>-linux-gnu}, e.g., \prog{gcc-powerpc64le-linux-gnu} for little-endian POWER8.
...
APPLICATIONS  INSTALL              ISSUES  musl-1.10.diff  patches  tool
common        install_compiler.py  lib     musl-1.16.diff  README   util
\end{lstlisting}
...
usage: install_compiler.py [-h] [--base-path BASE_PATH]
                           ...
                           [--skip-prereq-check] [--skip-llvm-clang-install]
                           [--skip-binutils-install]
                           [--skip-libraries-install] [--skip-tools-install]
                           [--skip-utils-install] [--skip-namespace]
                           [--install-call-info-library] [--targets TARGETS]
